...
class Contacts:
    ...
    def validate(self):
        ...
        existing_contact = next(
            filter(
                lambda c: c.id != self.id and c.email == self.email,
                Contacts.db.values(),
            ),
            None,
        )
    ...
    def save(self):
        if not self.validate():
            return False
        if self.id is None:
            if len(Contacts.db) == 0:
                max_id = 1
            else:
                max_id = max(contact.id for contact in Contacts.db.values())
            self.id = max_id + 1
            Contacts.db[self.id] = self
        Contacts.save_db()
        return True

    def delete(self):
        del Contacts.db[self.id]
        Contacts.save_db()
    ...
    @classmethod
    def load_db(cls):
        with open("model/db.json", "r") as contacts_file:
            contacts = json.load(contacts_file)
            cls.db.clear()
            for c in contacts:
                cls.db[c["id"]] = Contacts(
                    c["id"], c["first"], c["last"], c["phone"], c["email"]
                )

    @staticmethod
    def save_db():
        out_arr = [c.__dict__ for c in Contacts.db.values()]
